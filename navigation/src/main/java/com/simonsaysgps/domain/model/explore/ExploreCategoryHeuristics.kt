...
object ExploreCategoryHeuristics {
    fun relevance(category: ExploreCategory, candidate: ExploreCandidate): Double {
        ...
        return when (category) {
            ...
            ExploreCategory.OPEN_NOW -> if (candidate.openNow == true || candidate.primaryEvent != null) 1.0 else 0.35
            ...
            ExploreCategory.NEW -> if (candidate.recentlyOpenedOrTrending || ExploreFacet.NEW in facets || ExploreFacet.TRENDING in facets || candidate.confidenceSignals.any { it.label.equals("new", ignoreCase = true) }) 1.0 else 0.25
            ...
            ExploreCategory.HAVING_A_SALE -> if (candidate.primaryPromotion != null || ExploreFacet.SALE in facets) 1.0 else 0.2
        }.coerceIn(0.0, 1.0)
    }
